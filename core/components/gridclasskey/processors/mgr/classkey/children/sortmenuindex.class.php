 ...
 */
class GCKClasskeyChildrenSortProcessor extends modProcessor
{
    ...
    public function initialize()
    {
        ...
        $this->children = $this->modx->getCollection('modResource', $c);
        if (!$this->children) {
    ...
    public function process()
    {
        ...
        $targetMenuindex = $this->modx->getObject('modResource', $scriptProperties['targetId'])->get('menuindex');
        /**
        ...
        $c->sortby('menuindex', $scriptProperties['sortdir']);
        $c->sortby('id', $scriptProperties['sortdir']);
        $movingRes       = $this->modx->getCollection('modResource', $c);
        ...
        foreach ($movingRes as $res) {
            ...
            $c->sortby('menuindex', $scriptProperties['sortdir']);
            $c->sortby('id', $scriptProperties['sortdir']);
            $affectedRes = $this->modx->getCollection('modResource', $c);
...
return 'GCKClasskeyChildrenSortProcessor';
