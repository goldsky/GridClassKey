...
class GCKClasskeyChildrenBatchActionsProcessor extends modProcessor
{
    ...
    public function initialize()
    {
        ...
        if ($scriptProperties['action-range'] === 'selected') {
            $this->ids = json_decode($scriptProperties['action-selected-range'], 1);
            if (empty($this->ids)) {
                return $this->modx->lexicon('gridclasskey.empty_return_err');
            }
        }
    ...
    public function process()
    {
        ...
        if (!$collection) {
            return $this->failure(__LINE__.': '.$this->modx->lexicon('gridclasskey.empty_return_err'));
        }
        ...
        foreach ($collection as $item) {
            ...
            if (!empty($scriptProperties['action-showintree'])) {
                if ($scriptProperties['action-showintree'] === 'show') {
                    $item->set('show_in_tree', 1);
                } else {
                    $item->set('show_in_tree', 0);
                }
            }
...
return 'GCKClasskeyChildrenBatchActionsProcessor';
